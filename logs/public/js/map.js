...
function loadBoundaries() {
    ...
    if (window.mapData.townshipData) {
        boundaryLayers.townships.clearLayers();
        L.geoJSON(window.mapData.townshipData, {
            style: () => ({
                ...
                opacity: 0.6,
                fillOpacity: 0.05,
                dashArray: currentBoundaryStyle === 'dashed' ? '3, 3' : currentBoundaryStyle === 'dotted' ? '1, 3' : null
            }),
    ...
    if (window.mapData.villageData) {
        boundaryLayers.villages.clearLayers();
    ...
    if (window.mapData.electricData) {
        boundaryLayers.electric.clearLayers();
    ...
    if (window.mapData.fireData) {
        boundaryLayers.fire.clearLayers();
    ...
    if (window.mapData.emsData) {
        boundaryLayers.ems.clearLayers();
    ...
    if (window.mapData.schoolData) {
        boundaryLayers.school.clearLayers();
    ...
    if (window.mapData.telephoneData) {
        boundaryLayers.telephone.clearLayers();
